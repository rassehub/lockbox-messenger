...
describe('WebSocket Server', () => {
  ...
  it('accepts authenticated connections', async () => {
    ...
    await request(baseUrl).post('/auth/register').send({ username, phoneNumber, password: 'pw' }).expect(201);
    const loginRes = await request(baseUrl).post('/auth/login').send({ phoneNumber, password: 'pw' }).expect(200);
