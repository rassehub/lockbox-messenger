...
jest.mock('@/db', () => ({
  getRepository: jest.fn(() => ({
    findOne: jest.fn().mockResolvedValue({ phoneNumer: '12345', password_hash: 'password' }),
  })),
}));
...
describe ('Auth Controller', () => {
  it('should log in successfully with valid credentials', async () => {
    const req = {
      body: { phoneNumer: '12345', password: 'password' },
      session: {}
    } as any;
    const res = {
  ...
  it('should fail to log in with invalid credentials', async () => {
    const req = {
      body: { phoneNumer: '12345', password: 'wrongpassword' },
      session: {}
    } as any;
    const res = {
      status: jest.fn().mockReturnThis(),
